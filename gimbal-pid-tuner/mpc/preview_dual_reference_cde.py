"""Preview the frozen C/D/E reversal-plus-multisine dual-reference signal.
...
DEFAULT_PLAN = Path(__file__).with_name('dual_reference_excitation_plan_cde.json')
PHASES = ('C', 'D', 'E')
AXES = ('big', 'small')
...
def validate(plan):
    if (plan.get('schema_version') != 2
            or plan.get('status') != 'FROZEN_E_INDEPENDENT_VALIDATION_PLAN'
            or plan.get('hardware_execution_authorized') is not False):
        raise ValueError('C/D/E preview requires a frozen hardware-disabled plan')
    ...
    for axis in AXES:
        ...
        if (len(harmonics) != len(amplitudes) or not harmonics
                ...
                or sum(amplitudes) > item['peak_budget_deg'] + 1e-12):
    ...
    if (gates['hardware_takeover_allowed'] is not False
            or gates['E_role'].find('no coefficient') < 0):
        raise ValueError('E must remain independent and hardware-disabled')
...
def audit(plan):
    ...
    result = {
        'status': 'FROZEN_C_D_E_REFERENCE_PREVIEW',
        ...
        'warnings': [
            ...
            'E phases were selected from reference inputs only; no measured C/D output or residual was used.',
    ...
    result['same_axis_pairwise_correlation'] = {}
    for first, second in (('C', 'D'), ('C', 'E'), ('D', 'E')):
        result['same_axis_pairwise_correlation'][first+'_'+second] = {
            axis: float(np.corrcoef(
                generated[first][0][active_start:active_end, column],
                generated[second][0][active_start:active_end, column])[0, 1])
            for column, axis in enumerate(AXES)
        }
    ...
    checks += [
        abs(value) <= limits['maximum_abs_same_axis_prior_phase_correlation']
        for pair in result['same_axis_pairwise_correlation'].values()
