"""Evaluate E once against the exact C-only frozen closed-loop model.

The validation policy, C report hash and thresholds are fixed before E is
collected. E never refits the motion model. Hardware takeover stays disabled.
"""
...
from identify_e_reference_offline import (ACTIVE_END, AXES, BASELINE, DT,
...
def validate(c_report_path, e_capture):
    ...
    e_report, data = load_capture(e_capture)
    if e_report['capture_status'] != 'accepted' or e_report['audit']['phase_set'] != 'E':
        raise ValueError('input is not an accepted phase-E capture')
    ...
    if simulated is None:
        ...
        for name, start, end in (
                ('E_excitation', BASELINE, ACTIVE_END),
                ('E_settle', ACTIVE_END, len(measured)),
                ('E_continuous', BASELINE, len(measured))):
        ...
        joint_improvement = segments['E_excitation']['improvement_fraction'][:2]
        primary = {
            'finite_full_prediction': True,
            'minimum_per_joint_improvement_required': MINIMUM_JOINT_IMPROVEMENT,
            'E_excitation_joint_improvement_fraction': joint_improvement,
            'segments': segments,
            'residual_structure_E_excitation': _residual_structure(
    ...
    e_current = fit_command_current(data['command'], data['current'])
    comparisons = {}
    for axis in AXES:
        c_item = c_report['frozen_C_command_to_current'][axis]
        e_item = e_current[axis]
        relative_gain = abs(e_item['gain_current_raw_per_command_raw']-
                            c_item['gain_current_raw_per_command_raw']) / max(
                                abs(c_item['gain_current_raw_per_command_raw']), 1e-12)
        delay_difference = abs(e_item['delay_samples']-c_item['delay_samples'])
        comparisons[axis] = {
            'C_gain': c_item['gain_current_raw_per_command_raw'],
            'E_gain': e_item['gain_current_raw_per_command_raw'],
            ...
            'E_delay_samples': e_item['delay_samples'],
    ...
    return {
        'validation_status': ('passed_frozen_C_on_independent_E'
                              if overall else 'rejected_frozen_C_on_independent_E'),
        'model_status': ('closed_loop_candidate_passed_C_E_gates'
                         if overall else 'not_identified'),
        'hardware_takeover_allowed': False,
        'policy_frozen_before_E': {
            ...
            'E_motion_model_refit_allowed': False,
        },
        'E_capture_audit': e_report['audit'],
        'primary_frozen_C_motion_validation': primary,
        'post_primary_E_command_to_current_diagnostic': {
            'E_fit': e_current,
            'C_E_comparison': comparisons,
...
def main():
    ...
    parser.add_argument('e_capture', type=Path)
    parser.add_argument('--c-report', type=Path, required=True)
    parser.add_argument('--output', type=Path, required=True)
    args = parser.parse_args()
    if args.output.exists():
        parser.error('output must not exist')
    report = validate(args.c_report, args.e_capture)
