...
volatile BMI088_ReadDiagTypeDef BMI088_ReadDiag;
static HAL_StatusTypeDef s_bmi_read_status;
...
void BMI088_GetOffset() {
    ...
    /* Persistent calibration is disabled; use deterministic board defaults.
     * Never interpret an unread Flash buffer as calibration data. */
    bmi088->gyro_offset[0] = BMI088_GxOFFSET;
    bmi088->gyro_offset[1] = BMI088_GyOFFSET;
    bmi088->gyro_offset[2] = BMI088_GzOFFSET;
    bmi088->gNorm = BMI088_gNORM;
...
/**
  ...
  * @param      NULL
  * @retval     Offline or not閿熸枻鎷? is offline閿熸枻鎷? is not閿熸枻鎷?  */
...
/**
  * @brief      BMI088 decode data function    閿熸枻鎷稦or BMI088)
  * @param      NULL
  * @retval     NULL
  */
static uint8_t BMI088_ReadChecked(GPIO_GPIOTypeDef *cs, uint8_t reg,
    uint8_t *data, uint8_t length, uint8_t dummy) {
    uint8_t tx, rx = 0U;
    uint8_t total = (uint8_t)(1U + dummy + length);
    HAL_StatusTypeDef status;
    GPIO_Reset(cs);
    /* Preserve the previously working byte-at-a-time bus timing, but propagate
     * every HAL failure. CS stays asserted across address, dummy and data. */
    for (uint8_t i = 0; i < total; ++i) {
        tx = i <= dummy ? (reg | 0x80U) : 0x55U;
        status = HAL_SPI_TransmitReceive(Const_BMI088_SPI_HANDLER, &tx, &rx, 1U, 10U);
        if (status != HAL_OK) {
            s_bmi_read_status = status;
            GPIO_Set(cs);
            return 0U;
        }
        if (i > dummy) data[i - 1U - dummy] = rx;
    }
    GPIO_Set(cs);
    return 1U;
}
...
void BMI088_BMI088DecodeData() {
    BMI088_BMI088DataTypeDef *bmi088 = BMI088_GetBMI088DataPtr();
    uint8_t buff[8], accel[6], gyro[8], temp[2], chip_id;
    int16_t raw_temp;
    BMI088_ReadDiagTypeDef diag = BMI088_ReadDiag;

    /* Publish nothing unless every synchronous transaction succeeds. */
    uint8_t stage = 1U;
    s_bmi_read_status = HAL_OK;
    if (!BMI088_ReadChecked(CS_ACCEL, BMI088_ACC_CHIP_ID, &chip_id, 1U, 1U)) goto failed;
    diag.acc_id = chip_id;
    if (chip_id != BMI088_ACC_CHIP_ID_VALUE) goto failed;
    stage = 2U;
    if (!BMI088_ReadChecked(CS_ACCEL, BMI088_ACCEL_XOUT_L, accel, 6U, 1U)) goto failed;
    stage = 3U;
    if (!BMI088_ReadChecked(CS_GYRO, BMI088_GYRO_CHIP_ID, gyro, 8U, 0U)) goto failed;
    diag.gyro_id = gyro[0];
    if (gyro[0] != BMI088_GYRO_CHIP_ID_VALUE) goto failed;
    stage = 4U;
    if (!BMI088_ReadChecked(CS_ACCEL, BMI088_TEMP_M, temp, 2U, 1U)) goto failed;
    memcpy(buff, accel, sizeof(accel));
    ...
    ++diag.succeeded;
    diag.failed_stage = 0U;
    goto publish;
failed:
    ++diag.failed;
    diag.failed_stage = stage;
    bmi088->state = BMI088_STATE_ERROR;
publish:
    {
        uint32_t primask = __get_PRIMASK();
        diag.hal_status = (uint8_t)s_bmi_read_status;
        __disable_irq();
        BMI088_ReadDiag = diag;
...
/**
  * @brief      BMI088 decode temperature data function    閿熸枻鎷稦or BMI088)
