...
void Motor_InitAllMotors() {
...
#if BOARD_HAS_GIMBAL
    // 云台 Yaw
    // 大 yaw(6020)：can1，控制标识符 0x2FF，反馈标识符 0x209（电机id=5）
    // 小 yaw(6020)：can2，保持原控制/反馈映射
    //
    // 说明：Motor_SendMotorGroupOutput() 仍按 8 字节把 motor_handle[0..3] 打进同一帧
    // txdata[0..7]；Motor_EncoderDecodeCallback() 依据 motor_handle[j]->id 来匹配 RX StdId。
    Motor_groupHandle[0] = &Motor_Big_YawMotors;
    Motor_InitMotorGroup(&Motor_Big_YawMotors, Motor_TYPE_RM6020, 3, &hcan1, 0x2FF);
    Motor_InitMotor(&Motor_Big_YawMotor, Motor_TYPE_RM6020, 0x209, 0.1, gm6020_encoder_callback);
    // 发送到 0x2FF 帧的槽位：本电机 fdb StdId=0x209 对应 motor_handle[0]
    Motor_Big_YawMotors.motor_handle[0] = &Motor_Big_YawMotor;

    Motor_groupHandle[1] = &Motor_Small_YawMotors;
    Motor_InitMotorGroup(&Motor_Small_YawMotors, Motor_TYPE_RM6020, 2, &hcan2, 0x1FF);
    Motor_InitMotor(&Motor_Small_YawMotor, Motor_TYPE_RM6020, 0x206, 0.1, gm6020_encoder_callback);
    Motor_Small_YawMotors.motor_handle[1] = &Motor_Small_YawMotor;


    Motor_groupHandle[2] = &Motor_ShootMotors;//发射机构电机组(包含两个3508摩擦轮和一个2006拨弹电机)
	...
	Motor_groupHandle[3] = &Motor_ChassisMotors;//底盘轮向电机组(四个轮向3508)
	...
	Motor_groupHandle[4] = &Motor_ChassisSteerMotors;//底盘舵向电机组(四个舵向6020)
...
uint8_t Motor_IsMotorOffline(Motor_MotorTypeDef* pmotor) {
    if (pmotor == NULL) return 0;
    if (pmotor->type == Motor_TYPE_NOT_CONNECTED) return 0;
    /* CAN IRQ updates both the timestamp and online flag. Keep the decision
     * atomic so a newer timestamp cannot underflow against an older 'now'. */
    uint32_t mask = __get_PRIMASK();
    uint8_t offline;
    __disable_irq();
    uint32_t now = HAL_GetTick();
    offline = (uint8_t)(pmotor->init == 0 ||
        (now - pmotor->last_update_time) > Const_Motor_MOTOR_OFFLINE_TIME);
    pmotor->is_online = (uint8_t)!offline;
    __set_PRIMASK(mask);
    return offline;
}
...
uint8_t Motor_IsMotorFeedbackFresh(Motor_MotorTypeDef* pmotor, uint32_t max_age_ms) {
    ...
    uint32_t now;
    uint8_t fresh;
    if (pmotor == NULL) return 0U;
    if (pmotor->type == Motor_TYPE_NOT_CONNECTED) return 1U;
    /* Snapshot the clock and IRQ-written timestamp in one critical section.
     * Reading now first and allowing a CAN callback to publish now+1 before
     * subtraction makes the unsigned age underflow and falsely look stale. */
    mask = __get_PRIMASK();
    __disable_irq();
    now = HAL_GetTick();
    fresh = (uint8_t)(pmotor->init != 0U &&
        (now - pmotor->last_update_time) <= max_age_ms);
    __set_PRIMASK(mask);
    return fresh;
}
