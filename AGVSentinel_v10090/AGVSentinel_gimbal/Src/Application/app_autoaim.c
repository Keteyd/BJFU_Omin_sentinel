...
static UartMux_State s_uart_mux;
...
static uint8_t AutoAim_MuxValid(const uint8_t *data, uint16_t len)
{
    ...
    return (uint8_t)(len == ChariotRecognition_data_len &&
        data[VISION_FRAME_END_OFFSET] == 0x45U && AutoAim_VerifyFrameCRC8(data, len));
}
...
static void AutoAim_MuxDispatch(const uint8_t *data, uint16_t len)
{
    if (data[0] == 0xa6U) AttitudeLink_Receive(data, len);
        ...
        GetVisionData(&visionDataGet, frame);
    }
}

void AutoAim_ProcessUartData(uint8_t *data, uint16_t len)
{
    UartMux_Push(&s_uart_mux, data, len, HAL_GetTick(), AutoAim_MuxValid, AutoAim_MuxDispatch);
}
