...
static volatile uint32_t s_ins_update_count;
static volatile uint32_t s_ins_last_update_time;
static volatile uint32_t s_ins_valid_streak;
static INS_Observation s_ins_observation;
volatile uint32_t INS_DiagReadRejected, INS_DiagDtRejected;
volatile float INS_DiagLastDtS;

void INS_ReadObservation(INS_Observation *observation) {
    uint32_t primask = __get_PRIMASK();
    __disable_irq();
    *observation = s_ins_observation;
    __set_PRIMASK(primask);
}

static void Param_Correction(INS_DataTypeDef *param, float gyro[3],
                             float accel[3]);
    ...
    forever {        
        ...
        if ((count % 1) == 0) {
            ...
            if (bmi088->state != BMI088_STATE_CONNECTED ||
                ...
                if (bmi088->state != BMI088_STATE_CONNECTED) ++INS_DiagReadRejected;
                if (!isfinite(dt) || dt <= 0.0f || dt > 0.01f) ++INS_DiagDtRejected;
                s_ins_valid_streak = 0U;
                osDelay(1U);
                continue;
            }
            ...
            s_ins_last_update_time = HAL_GetTick();
            if (s_ins_update_count < UINT32_MAX) {
                s_ins_update_count++;
            }
            {
                INS_Observation next;
                uint32_t primask;
                next.yaw_deg = INS.YawTotalAngle;
                next.roll_deg = INS.Roll;
                next.pitch_deg = INS.Pitch;
                memcpy(next.gyro, INS.Gyro, sizeof(next.gyro));
                next.dt_s = dt;
                ...
                if (s_ins_valid_streak < 20U) ++s_ins_valid_streak;
                __set_PRIMASK(primask);
            }
        }
        count++;
...
uint8_t INS_IsReady(void) {
    ...
    if (bmi088 == NULL || s_ins_valid_streak < 20U ||
        (now - s_ins_last_update_time) > 10U) {
        return 0U;
    }

    /* PENDING is normal while the next synchronous SPI sample is in progress. */
    return (uint8_t)(bmi088->state == BMI088_STATE_CONNECTED ||
                     bmi088->state == BMI088_STATE_PENDING);
}

void INS_Init() {
    s_ins_valid_streak = 0U;
    memset(&s_ins_observation, 0, sizeof(s_ins_observation));
    s_ins_update_count = 0U;
    s_ins_last_update_time = 0U;
    Param.scale[X_INS] = 1;
    Param.scale[Y_INS] = 1;
    Param.scale[Z_INS] = 1;
    Param.Yaw = 0;
