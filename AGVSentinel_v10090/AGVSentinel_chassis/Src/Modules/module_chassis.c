...
static ChassisRamp_StateTypeDef s_body_speed_ramp;
static uint32_t s_ramp_last_tick;
static uint8_t s_ramp_tick_valid;

static const ChassisRamp_ConfigTypeDef s_body_speed_ramp_config = {
    CHASSIS_TRANSLATION_ACCEL_PER_S,
    CHASSIS_TRANSLATION_DECEL_PER_S,
    CHASSIS_TRANSLATION_STOP_PER_S,
    CHASSIS_YAW_ACCEL_PER_S,
    CHASSIS_YAW_DECEL_PER_S,
    CHASSIS_YAW_STOP_PER_S,
    CHASSIS_RAMP_MAX_DT_S
};
...
void Chassis_InitChassis(void)
{
    ...
    ChassisRamp_Reset(&s_body_speed_ramp);
    s_ramp_last_tick = 0U;
    s_ramp_tick_valid = 0U;
...
void Chassis_SetChassisRef(float rc_vx, float rc_vy, float rc_wz)
{
    ...
    switch (Chassis_StatusData.chassis_mode) {
        case Chassis_SEP:
            ...
            break;
        case Chassis_GIMBAL:
            if (sqrtf(rc_vx * rc_vx + rc_vy * rc_vy) <=
                CHASSIS_GIMBAL_TRANSLATION_DEADBAND_COUNTS) {
                rc_vx = 0.0f;
                rc_vy = 0.0f;
            }
        ...
        default:
            ...
            Chassis_StatusData.Chassis_TargetVx = 0.0f;
            Chassis_StatusData.Chassis_TargetVy = 0.0f;
            Chassis_StatusData.Chassis_TargetWz = 0.0f;
    ...
    Chassis_StatusData.Chassis_TargetVx = vx;
    Chassis_StatusData.Chassis_TargetVy = vy;
    Chassis_StatusData.Chassis_TargetWz = wz;
    now = HAL_GetTick();
    if (s_ramp_tick_valid) {
        dt_s = (float)(now - s_ramp_last_tick) * 0.001f;
    } else {
        s_ramp_tick_valid = 1U;
    }
    s_ramp_last_tick = now;
    ChassisRamp_Step(&s_body_speed_ramp, vx, vy, wz, dt_s,
                     &s_body_speed_ramp_config);
    vx = s_body_speed_ramp.vx;
    vy = s_body_speed_ramp.vy;
    wz = s_body_speed_ramp.wz;
